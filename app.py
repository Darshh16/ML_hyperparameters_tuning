...
st.markdown("""
    ...
    - Train on famous datasets like Iris, Wine, Breast Cancer, and more
""")
...
with st.sidebar:
    ...
    if algorithm == "Random Forest":
        ...
        hyperparams['solver'] = st.selectbox("Solver", ['lbfgs', 'liblinear', 'saga'])
        
    elif algorithm == "Gradient Boosting":
        hyperparams['n_estimators'] = st.slider("Number of Estimators", 10, 500, 100)
        hyperparams['learning_rate'] = st.slider("Learning Rate", 0.001, 1.0, 0.1)
        hyperparams['max_depth'] = st.slider("Max Depth", 1, 15, 3)
        hyperparams['min_samples_split'] = st.slider("Min Samples Split", 2, 20, 2)
        hyperparams['subsample'] = st.slider("Subsample Ratio", 0.1, 1.0, 1.0)
        
    elif algorithm == "SGD Classifier" and task_type_lower == "classification":
        hyperparams['loss'] = st.selectbox("Loss Function", ['hinge', 'log_loss', 'squared_hinge'])
        hyperparams['alpha'] = st.slider("Alpha (Learning Rate)", 0.00001, 0.01, 0.0001, log=True)
        hyperparams['max_iter'] = st.slider("Max Iterations", 100, 2000, 1000)
        
    elif algorithm == "SGD Regressor" and task_type_lower == "regression":
        hyperparams['loss'] = st.selectbox("Loss Function", ['squared_error', 'huber', 'epsilon_insensitive'])
        hyperparams['alpha'] = st.slider("Alpha (Learning Rate)", 0.00001, 0.01, 0.0001, log=True)
        hyperparams['max_iter'] = st.slider("Max Iterations", 100, 2000, 1000)
        
    elif algorithm == "AdaBoost":
        hyperparams['n_estimators'] = st.slider("Number of Estimators", 10, 200, 50)
        hyperparams['learning_rate'] = st.slider("Learning Rate", 0.1, 2.0, 1.0)
        
    elif algorithm == "Linear Regression":
        st.info("Linear Regression has no adjustable hyperparameters")
